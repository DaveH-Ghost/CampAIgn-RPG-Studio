...
**CampAIgn RPG Studio 1.3.0** — reference GM web app for [CampAIgn-RPG-Engine](https://github.com/DaveH-Ghost/CampAIgn-RPG-Engine) (`campaign-rpg-engine>=1.3.0`).
...
`pyproject.toml` pins **`campaign-rpg-engine>=1.3.0`** and `[tool.uv.sources]` points at the sibling engine checkout. `uv sync` installs the engine editable — engine changes are picked up without reinstalling.
